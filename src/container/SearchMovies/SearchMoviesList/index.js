...
function SearchMoviesList(props) {
  const [page, setPage] = useState(1);
  const [moviesData, setMoviesData] = useState([]);
  const [lazyLoading, setLazyLoading] = useState(false);
  const {route} = props;
  const {title, imageBaseUrl, posterSize} = route.params;
  useEffect(() => {
    _fetchMoviesData();
  }, []);
  ...
  const _fetchMoviesDataMore = () => {
    ...
    let nextPage = page + 1;
    setPage(nextPage);
    const url = `https://api.themoviedb.org/3/search/movie?query=${title}&include_adult=false&language=en-US&page=${nextPage}`;
    const options = {
      method: 'GET',
      headers: {
        accept: 'application/json',
